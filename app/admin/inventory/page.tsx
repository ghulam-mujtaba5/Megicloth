...
const handleSaveStock = async (productId: string) => {
  ...
  const getStockStatus = (stock: number): { label: string; color: 'success' | 'warning' | 'error' } => {
    ...
    if (stock <= LOW_STOCK_THRESHOLD) return { label: 'Low Stock', color: 'warning' };
                ...
                <TableCell>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <TextField
                      ...
                      size="small"
                      variant="outlined"
                      value={stockLevels[product.id] ?? product.stock}
                      onChange={(e) => handleStockChange(product.id, e.target.value)}
                      sx={{ width: '100px', mr: 1 }}
                    />
                    <IconButton onClick={() => handleSaveStock(product.id)} size="small">
                      <Save />
                    </IconButton>
                  </Box>
                </TableCell>
